...
async def main(page: ft.Page):
    ...
    # ==========================================
    lbl_clock_time = ft.Text(
        "00:00:00",
        size=50,
        weight=ft.FontWeight.BOLD
    )

    lbl_clock_date = ft.Text(
        "",
        size=16,
    ...
    btn_chrono_reset = ft.OutlinedButton(
        content=ft.Text("Reiniciar")
    )

    async def toggle_chrono(e):
        chrono_state["running"] = not chrono_state["running"]

        btn_chrono_start.content.value = (
            "Pausar"
            if chrono_state["running"]
            else "Iniciar"
        )

        btn_chrono_start.bgcolor = (
            ft.Colors.RED
            if chrono_state["running"]
            else ft.Colors.BLUE
        )
    ...
    gallery_picker = ft.FilePicker(
        on_result=display_gallery
    )
    ...
    # ==========================================
    list_files = ft.ListView(
        expand=True,
        spacing=4
    )

    async def open_files(e):
        await files_picker.pick_files_async(
            allow_multiple=True
        )
    ...
    files_picker = ft.FilePicker(
        on_result=display_files
    )
        ...
        while True:
            try:
                ...
                if chrono_state["running"]:
                    ...
                    lbl_chrono.value = (
                        f"{chrono_state['h']:02d}:"
                        f"{chrono_state['m']:02d}:"
                        f"{chrono_state['s']:02d}"
                    )
    ...
    # OVERLAYS
    # ==========================================
    page.overlay.append(gallery_picker)
